...
struct granice {int donja_granica,gornja_granica;};
...
void *merge_sort(void *arg) {
    ...
    if (gornja_granica-donja_granica<2) pthread_exit(NULL);
    pthread_t lijeva_nit;
    pthread_t desna_nit;
    struct granice argument_lijevog={donja_granica,sredina_niza};
    struct granice argument_desnog={sredina_niza,gornja_granica};
...
int main() {
    ...
    for (int i=0; i<n; i++)
        scanf("%d",original+i);
    struct granice pocetne_granice={0,n};
