...
int main() {
    ...
    cout <<"s\tC++ \"sort\" directive\tHybridSort in C\tHybridSort in AEC" <<endl;
    vector<int> polje;
    ...
    for (int i=0; i<=15000; i+=50) {
        ...
        system("./hsortInC.elf64 < ulaz.txt > /dev/null");
        clock_t treci=getTimeInMiliseconds();
        cout <<log(treci-drugi) <<'\t' <<flush;
        system("wine hsort.exe < ulaz.txt > /dev/null");
        clock_t cetvrti=getTimeInMiliseconds();
        cout <<log(cetvrti-treci) <<endl;
    }
}
